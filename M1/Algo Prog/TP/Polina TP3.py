...
# exercice 3 (rectangle blanc) VERSION 2
def rectangle_blanc(haut,larg,y1,y2,x1,x2):
	for y in range(haut):
		for x in range(larg):
...
# exercices 4 et 5 (numéro de bandes noirs et blanches)
''' num_bande=x//larg_bande
noire: num_bande%2!=0
blanche: num_bande%2==0 '''

# exercice 6 (rayures verticales avec comp)
...
graph.plot(y,x)
'''
#  exercice 9 (damier avec comp) VERSION 1
...
# exercice 9 (damier avec comp) VERSION 2
def damier_new(haut,larg,larg_bande):
	for y in range(haut):
		for x in range(larg):
			x_i=x%(2*larg_bande)
...
# exercice 10 (damier multicolore)
